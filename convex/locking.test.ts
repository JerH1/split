...
describe("bill locking", () => {
  describe("setLocked", () => {
    ...
    it("rejects someone using the host's public ID without their secret", async () => {
      const t = convexTest(schema);
      const { sessionId, hostId } = await setup(t);
  ...
  describe("guards on a locked bill", () => {
    ...
    it("refuses claiming for everyone", async () => {
      const t = convexTest(schema);
      const { sessionId, hostId, itemId } = await setup(t, true);
    ...
    it("refuses a ready-state change", async () => {
      const t = convexTest(schema);
