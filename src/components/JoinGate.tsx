...
export default function JoinGate({ session, onJoined }: JoinGateProps) {
  const t = useT();
  useDocumentTitle(t("join.documentTitle", { code: session.code }));
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    try {
      ...
      let errorMessage = t("join.failed");
    ...
    <div className="relative mx-auto max-w-md">
      ...
      <div className="flex flex-col items-center gap-2 border-b-2 border-brand bg-surface px-6 pb-4 pt-12">
        ...
        <p className="text-sm text-ink-2">
          {t("join.hostedBy", { name: session.hostName })}
        </p>
      </div>
      ...
      <div className="space-y-6 p-6">
        <div className="text-center">
          ...
          <p className="mt-1 text-ink-2">{t("join.body")}</p>
        </div>
